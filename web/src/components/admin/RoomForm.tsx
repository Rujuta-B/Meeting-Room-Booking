...
import { listAttributes } from '../../api/rooms';
import { FLOOR_OPTIONS, formatFloorLabel } from '../../lib/floor';
import { MIN_CAPACITY, MAX_CAPACITY } from '../../lib/capacity';
import { Select } from '../Select';
import type { Room, RoomAttribute, CreateRoomInput } from '../../types/room';

export interface RoomFormProps {
  initial?: Room | undefined;
  onSubmit: (input: CreateRoomInput) => void;
  submitting: boolean;
  serverError?: string | undefined;
}

export function RoomForm({ initial, onSubmit, submitting, serverError }: RoomFormProps) {
  const [name, setName] = useState(initial?.name ?? '');
  const [floor, setFloor] = useState(initial?.floor ?? 1);
  const [capacity, setCapacity] = useState<number | string>(initial?.capacity ?? 4);
  const [capacityError, setCapacityError] = useState<string | null>(null);
  const [knownAttributes, setKnownAttributes] = useState<RoomAttribute[]>([]);
