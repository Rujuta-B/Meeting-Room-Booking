// src/routes/BookingFormPage.tsx
//
...
// gracefully end to end - see BookingConflictBanner.tsx for why a 409 gets
...
import { BookingConflictBanner } from '../components/bookings/BookingConflictBanner';
...
export function BookingFormPage() {
  ...
  const [refreshedRooms, setRefreshedRooms] = useState<AvailableRoom[] | null>(null);
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      if (err instanceof ApiError && err.code === 'BOOKING_CONFLICT') {
        ...
        setError(err.details?.errors?.[0]?.message ?? 'Could not create the booking. Please try again.');
      } else {
