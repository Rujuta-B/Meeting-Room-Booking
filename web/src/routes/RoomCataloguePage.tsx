// src/routes/RoomCataloguePage.tsx
//
...
import { listRooms, listAttributes } from '../api/rooms';
import { formatFloorLabel } from '../lib/floor';
import { MIN_CAPACITY, MAX_CAPACITY } from '../lib/capacity';
import type { Room, RoomAttribute } from '../types/room';

export function RoomCataloguePage() {
  const [rooms, setRooms] = useState<Room[] | null>(null);
  const [attributes, setAttributes] = useState<RoomAttribute[]>([]);
  ...
  function clampCapacity(value: string): string {
    return value === '' ? '' : String(Math.min(Number(value), MAX_CAPACITY));
  }

  function enforceFloorOnBlur(value: string, setValue: (value: string) => void) {
